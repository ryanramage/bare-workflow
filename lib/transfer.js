  ...
  return new Promise((resolve, reject) => {
    ...
    parser.on('entry', (header, source, next) => {
      ...
      try {
        ...
        // something we want to reproduce on the host. Exactly ONE bit survives -- owner-execute --
        // because a distributable that comes back non-executable is not a distributable, and this
        // is the whole of what a build legitimately needs to express. Everything else (setuid,
        // setgid, sticky, group/other write) is discarded by construction rather than by masking,
        // so there is no arithmetic to get subtly wrong.
        const mode = header.mode & 0o100 ? 0o755 : 0o644
        ...
        source.on('end', () => {
          ...
          try {
            fs.writeFileSync(target, Buffer.concat(chunks), { mode })
            files++
            bytes += written
          } catch (err) {
            failed = failed || err
