...
// `machine` emits exactly the same argv as `container`: the VM boundary it names is a property of
// where the podman SERVICE runs, not something this process can ask for with a flag. Keeping it in
// the vocabulary means a spec can carry it through to the attestation.
const TIERS = ['microvm', 'machine', 'container']
...
function requiredHostEnv(spec, source) {
  // Built by copying keys that are set, never by naming them as literal properties -- see
  // lib/host-env.js for why that distinction is load bearing.
  const out = hostEnv(null, source || {})
  if (spec.scope === false) {
    // Without a systemd scope there is no session bus to talk to, so these are not just unnecessary
    // but misleading in the attestation's recorded argv.
    delete out.DBUS_SESSION_BUS_ADDRESS
    delete out.XDG_RUNTIME_DIR
  }
  return out
}
