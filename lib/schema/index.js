...
// Derived from the tier registry, never written out again. There are three places that need to know
// the tier vocabulary -- this one, `detect.TIERS` (the registry, with ranks) and `argv.TIERS` -- and
// duplicating it here already went wrong once: the `machine` tier was added to the registry and to
// argv but not here, so `--tier machine` worked while a workflow declaring `tier: machine` was
// rejected as an unknown tier. That is worse than a plain gap, because decision 8 says a workflow's
// declared tier is part of the reviewable contract: a Mac user could not state the boundary their
// build required. test/schema.js asserts all three lists agree.
const TIERS = require('../isolation/detect.js').TIERS.map((t) => t.name)
