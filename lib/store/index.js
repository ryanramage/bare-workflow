...
class Store {
  ...
  async put(name, dir, opts = {}) {
    ...
    const walk = (current, prefix) => {
      ...
      for (const entry of names) {
        ...
        // The execute bit is the one piece of mode an artifact legitimately carries: a distributable
        // that arrives non-executable is not a distributable. lib/transfer.js has already reduced
        // whatever came out of the sandbox to exactly this bit, so there is nothing left to sanitise.
        files.push({ abs, rel, size: stat.size, executable: !!(stat.mode & 0o100) })
        bytes += stat.size
      }
    }
    walk(root, '')
    ...
    // A stable digest over the tree, so the attestation binds to the BYTES rather than to a name.
    const entries = []
    for (const file of files) {
      const body = fs.readFileSync(file.abs)
      await this.drive.put(this._key(name, file.rel), body, { executable: file.executable })
      entries.push({ rel: file.rel, body, executable: file.executable })
    }

    return { name, files: files.length, bytes, digest: treeDigest(entries) }
  }
  ...
  async get(name, dir) {
    ...
    for await (const entry of this.drive.list(prefix)) {
      ...
      // `executable` is drive metadata, not a filesystem detail, so this works identically once the
      // farm swaps a Hyperdrive in behind the same surface.
      const executable = !!entry.value.executable
      fs.writeFileSync(target, buf, { mode: executable ? 0o755 : 0o644 })
      entries.push({ rel, body: buf, executable })
    ...
    // Recomputed on the way OUT, not carried alongside. That makes `get` an integrity check as well
    // as a copy, and it gives a consumer -- notably a publish job -- the digest of what it is about
    // to act on rather than a digest something else recorded earlier.
    return { name, files: entries.length, bytes, dir: dest, digest: treeDigest(entries) }
  }
...
// One digest function for both directions, because a digest that only agrees with itself is not
// doing anything. Three details are load-bearing:
//
//   * SORTED BY PATH, explicitly. `put` walks a directory tree depth-first while `get` reads keys
//     from a drive in lexicographic order, and those two orders genuinely differ -- for `a/b` and
//     `a.txt`, a walk yields `a/b` first while '.' sorts before '/'. Sorting here rather than
//     relying on either traversal is what makes producer and consumer agree.
//   * THE EXECUTE BIT IS INCLUDED. Two trees differing only in which files are runnable are
//     different artifacts, and one of them is broken.
//   * PATHS ARE RELATIVE, so the same tree digests the same on a different machine -- which is the
//     property a farm needs from it.
function treeDigest(entries) {
  ...
  const lines = entries
    ...
    .map(
      (e) =>
        crypto.createHash('sha256').update(e.body).digest('hex') +
        (e.executable ? '  x  ' : '  -  ') +
        e.rel
    )
  return (
...
module.exports = { Store, create, localStore, treeDigest, NAME }
