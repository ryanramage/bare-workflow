  ...
  return {
    ...
    isolation: {
      ...
      seccomp: isolation.seccompProfile
        ...
        : null,
      // The Seatbelt profile, for a native tier. Same principle as seccomp above -- the content is
      // the control, so the hash is what matters -- but it cannot reuse that field: the two are
      // different documents enforced by different mechanisms, and a consumer must not have to guess
      // which one a `profile` hash refers to.
      //
      // The launcher supplies the hash rather than a path to re-read, because this profile is
      // generated per job into the job's scratch directory and is deleted at teardown. Hashing the
      // bytes that were enforced is the point; a path that no longer exists is not.
      sandbox: isolation.sandboxProfile
        ? {
            path: isolation.sandboxProfile,
            sha256: isolation.sandboxProfileSha256 || fileSha256(isolation.sandboxProfile),
            workspace: isolation.workspace || null,
            toolchain: isolation.toolchain || []
          }
        : null,
      // What was actually enforced, and what could not be. A native tier has no cgroups, so a limits
      // block listing only the ulimits that worked would read as though the memory cap applied too.
      // `forPublish` below sets the precedent: a different shape beats a familiar one full of nulls.
      limits: isolation.limits || null,
      limitsUnenforceable: isolation.limitsUnenforceable || null,
