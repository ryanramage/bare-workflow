...
// `/w` is the workspace root for every CONTAINER tier, where the agent owns the filesystem and the
// path can simply be decided. A native tier cannot: it runs on the developer's own machine, where
// creating `/w` would need root and would be shared between jobs. So the root is a variable with `/w`
// as the default, and a launcher that needs a different one says so -- see `workspaceOf` below.
const DEFAULT_WORKSPACE = '/w'
...
function baseEnv(workspace) {
  return {
    ...
    TMPDIR: '/tmp',
    CI: 'true',
    // Exported so a workflow can be written once and run on any tier. Without it a step has to
    // hardcode `/w/cache`, which is correct in a container and wrong on a native tier where the
    // workspace is a per-job scratch directory -- and the failure is an npm cache miss, not
    // anything that mentions a path.
    BW_WORKSPACE: workspace
  }
}
...
class Sandbox extends EventEmitter {
  ...
  async prepare() {
    ...
    // Did the fd scan run at all? An agent older than hello v5 does not say, which is reported as
    // 'unknown' rather than assumed to be 'ok' -- the whole reason this field exists is that an
    // empty strayFds list means nothing on its own.
    //
    // This is surfaced rather than enforced here. sandbox.js does not know whether the launcher it
    // was handed claims isolation, and the test-only local launcher legitimately cannot scan on a
    // platform without procfs. The caller that DOES know -- the tier, or the lifecycle suite -- is
    // where "an isolated tier must have a working scan" belongs.
    this.fdScan = hello.fdScan || 'unknown: agent predates the fdScan field'

    // Where this sandbox's workspace lives. The launcher is the only thing that knows: a container
    // tier owns its filesystem and always answers `/w`, while a native tier has to use a per-job
    // scratch directory on the developer's own machine. Read after spawn so the launcher can decide
    // it per job rather than the driver assuming a path that only exists inside a container.
    this.workspace = this._handle.workspace || DEFAULT_WORKSPACE

    // Env floor overrides from the launcher. A container tier needs none -- the image puts npm on
    // /usr/bin and /tmp is a tmpfs -- but a native tier's toolchain lives wherever the host keeps
    // it, and its TMPDIR has to be inside the sandbox's writable area. Without this a seatbelt step
    // gets the container's PATH and dies with `npm: command not found`.
    //
    // A step's own `env:` still wins over these, exactly as it wins over the floor.
    this.launcherEnv = this._handle.env || {}
  ...
  exec(step, opts = {}) {
    ...
    duplex.write(
      protocol.frames.start({
        ...
        cwd: step.cwd || this.workspace + '/src',
        env: toEnvPairs({
          ...baseEnv(this.workspace),
          ...this.launcherEnv,
          [OUTPUT_ENV]: (opts.workspace || this.workspace) + '/' + protocol.OUTPUT_FILE,
          ...(step.env || {})
        }),
