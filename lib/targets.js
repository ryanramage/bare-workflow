...
// Targets whose packaging tooling we have NOT verified. Declared unbuildable rather than guessed at:
// claiming a capability we cannot honour fails late and confusingly.
//
// This is a hard decline, on every platform. It used to fall back to "buildable only natively", which
// was indistinguishable for `android-arm64` (it is native to no platform here) and became wrong the
// moment a darwin execution tier existed: the ios targets are native to darwin, so adding that tier
// would have claimed all three for free.
//
// The ios entries are the point. What the seatbelt tier actually demonstrated is that `codesign`
// produces a runnable arm64 **Mach-O executable** -- not an iOS app bundle, an SDK, a provisioning
// profile or a simulator runtime, none of which has been exercised here. "We are on a Mac, so iOS
// must work" is precisely the guess this list exists to refuse; `android-arm64`'s comment already
// said so and the reasoning transfers unchanged.
const UNVERIFIED = ['android-arm64', 'ios-arm64', 'ios-arm64-simulator', 'ios-x64-simulator']
...
const TIER_PLATFORM = {
  container: 'linux',
  // A podman-machine VM is a Linux guest, so the workload still executes on linux -- which is
  // exactly why naming this tier does NOT unlock darwin-arm64. The tier that would is a darwin one.
  machine: 'linux',
  microvm: 'linux',
  // ...and this is that tier. The single entry that legitimately makes buildableOn('darwin',
  // 'darwin-arm64') true, because a Seatbelt-confined build runs on macOS itself where `codesign`
  // exists.
  //
  // It must be added in the SAME change as the tier name, never after it. executionPlatform() below
  // falls back to the HOST platform for any tier name it does not recognise, so a `seatbelt` tier
  // registered in detect.js WITHOUT this line already reports execPlatform 'darwin' and unlocks
  // darwin-arm64 -- plus all three ios targets -- while nothing can actually execute it. Measured,
  // and the opposite of what "add the mapping last, once it works" would suggest. What gates the
  // capability is the PROBE reporting the tier unavailable, not the absence of this entry.
  seatbelt: 'darwin'
}
...
function executionPlatform(tiers, host) {
  ...
  if (!named.length) return leastCapablePlatform()
  return host
}

// The execution platform to assume when NOTHING is known about which tier will run: the one that
// promises the LEAST.
//
// This used to be "if every tier agrees, use that", which was the same thing while every tier ran
// Linux -- and silently stopped being the same thing the moment the seatbelt tier put 'darwin' in the
// registry. With two platforms present the old rule fell through to the host, and an unconfigured Mac
// went straight back to claiming darwin-arm64: the exact regression CLAUDE.md records as already
// fixed once, reintroduced by adding a tier rather than by touching this function.
//
// Ranking by capability rather than by tier rank keeps it honest without importing the rank table:
// the question here is "what can I promise before I know what I have", and the answer is the smallest
// set. On Linux both answers still coincide.
function leastCapablePlatform() {
  let best = null
  let fewest = Infinity
  for (const platform of [...TIER_PLATFORMS].sort()) {
    const count = TARGETS.filter((target) => buildableOn(platform, target)).length
    if (count < fewest) {
      fewest = count
      best = platform
    }
...
function buildableOn(platform, target) {
  ...
  if (UNVERIFIED.includes(target)) return false
...
function describe(opts = {}) {
  ...
  return {
    ...
    // UNVERIFIED targets are excluded on purpose: they are not a SIGNING gap, so routing them to a
    // Mac peer would not help, and listing them here would tell a farm the opposite. That is exactly
    // the "cannot build" versus "cannot sign" distinction this field exists to draw.
    unsignable: TARGETS.filter(
      (t) => !UNVERIFIED.includes(t) && !buildableOn(runsOn, t) && SIGNING_HOST[t]
    ),
    tiers,
