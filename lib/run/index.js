...
class TaskRun extends EventEmitter {
  constructor({
    ...
    // Resolved against the sandbox's own workspace once it is up -- a native tier's is a per-job
    // scratch directory, not `/w`. Left null here so `null` means "ask the sandbox" rather than
    // silently baking in a path that only exists inside a container.
  ...
  async execute() {
    ...
    try {
      await box.prepare()
      // The sandbox knows its workspace only once the launcher has told it, so this is the first
      // point at which a workdir can be resolved. An explicitly configured one still wins.
      if (!this.workdir) this.workdir = box.workspace + '/src'
      this.agentVersion = box.hello.agent
    ...
    return {
      ...
      steps: this.steps,
      outputs: this.outputs,
      // Carried on the RESULT, not just emitted as an event: the attestation has to record which
      // artifacts a task produced and their content digests, and an event is not a return value.
      artifacts: this.artifacts,
      agent: this.agentVersion || null
    }
  }
  ...
  async _putCache(box) {
    ...
    const cacheDest = box.workspace + '/cache'
    const put = await box.put(this.runMeta.cacheDir, cacheDest)
    this.emit('cache', { to: cacheDest, files: put.files, bytes: put.bytes })
  }
